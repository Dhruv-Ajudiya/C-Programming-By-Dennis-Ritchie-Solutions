 ...
 * This program also supports to print the top element of the stack without popping,
 * duplicate the top element of stack and swap top two elements of stack and clear the
 * stack.
 ...
 *	 operations and prints the result. It handles various cases to print top
 *	 element of the stack without popping, duplicate the top element of the stack
 *	 and swap top two elements of the stack and clear the stack.
 */
int main(void)
{
	...
	int iTopElement;		/* Store the top elements of stack to print */
	...
	printf("Use Reverse Polish Notation to enter the value\n"
		...
		"enter '/' for division\n"
		"enter '%%' for modulus\n"
		"enter '?' to print top of stack without popping\n"
		"enter 'D' to duplicate the last element of stack\n"
		"enter 'S' to swap top two elements of the stack\n"
	...
	while ((iOpType = getNextOp(cInputString)) != EOF) {
		...
		case '?':
			if (iStackPointer > 0) {
				printf("Enter the number of top elements to print: ");
				scanf("%d", &iTopElement);
				...
				if (iTopElement > iStackPointer)
					iTopElement = iStackPointer;

				printf("Top %d elements of the stack: ", iTopElement);
				for (i = 0; i < iTopElement; i++)
					printf("%.8g ", dStackValue[iStackPointer - 1 - i]);
				printf("\n");
			} else {
		...
		case 'D':
			if (iStackPointer > 0) {
				printf("Stack before duplicating the top element:\n");
				for (int i = iStackPointer - 1; i >= 0; i--)
					printf("%.8g ", dStackValue[i]);
				printf("\n");

				pushToStack(dStackValue[iStackPointer - 1]);

				printf("Stack after duplicating the top element:\n");
				for (int i = iStackPointer - 1; i >= 0; i--)
					printf("%.8g ", dStackValue[i]);
				printf("\n");
			} else {
			...
			break;
		/* Swap two elements of stack */
		case 'S':
			if (iStackPointer > 1) {
				printf("Stack before swapping:\n");
				for (i = iStackPointer - 1; i >= 0; i--)
					printf("%.8g ", dStackValue[i]);
				printf("\n");

				dTempValue = dStackValue[iStackPointer - 1];
				dStackValue[iStackPointer - 1] = dStackValue[iStackPointer - 2];
				dStackValue[iStackPointer - 2] = dTempValue;
				...
				printf("Error: Less than two values in stack, cannot swap\n");
			}
			break;
		/* Clear the stack */
		case 'C':
